...
d %>% select(date = "日付", pref = "都道府県名",
             case = "各地の感染者数_1日ごとの発表数") %>% 
  mutate(date = as.Date(date)) %>%
  filter(date > max(date) - 14) %>%
  group_by(pref) %>%
  mutate(ms7_case = slide_vec(case, .f = sum, .before = 6)) %>%
  filter(date %in% c(max(date), max(date) - 7))  -> df

ratio <- df$ms7_case[df$date == max(df$date)] / df$ms7_case[df$date == min(df$date)]


jp <- read_sf("https://github.com/igproj-fusion/R-gis/raw/main/japan.geojson")
jp$ratio <- ratio

breaks <- c(min(ratio), 1.0, max(ratio))
